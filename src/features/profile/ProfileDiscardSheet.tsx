...
export function ProfileDiscardSheet({
  ...
  return (
    <Drawer.Root open={open} onOpenChange={(nextOpen) => !nextOpen && onStay()} direction="bottom" modal dismissible>
      <Drawer.Portal>
        <Drawer.Overlay className={styles.discardBackdrop} />
        <Drawer.Content className={styles.discardSheet} aria-label="Discard profile changes confirmation">
